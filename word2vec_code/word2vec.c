...
#define MAX_STRING 100 //max word length
#define EXP_TABLE_SIZE 1000
#define MAX_EXP 6
#define MAX_SENTENCE_LENGTH 1000
#define MAX_CODE_LENGTH 40 //i.e. max path length
 ...
 */
const int vocab_hash_size = 30000000;  // Maximum 30 * 0.7 = 21M words in the vocabulary 出于减少哈希碰撞和性能考虑
 ...
 */
int *vocab_hash;//先算出词汇的hash值，vocab_hash[hash]就是word在vocab中的下标
 ...
 */
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100;//layer1_size　词嵌入的维度/hidden_size
 ...
 */
real alpha = 0.025, starting_alpha, sample = 1e-3;//后面训练时会做学习率衰减，starting_alpha用来记录起始学习率
 ...
 *　将原本的按词频的3/4次方采样转化为对UnigramTable中的元素均匀采样
...
void InitUnigramTable() {
  int a, i;
  double train_words_pow = 0;//归一化项，总词频的3/4次方
  ...
  for (a = 0; a < table_size; a++) {
    ...
    if (i >= vocab_size) i = vocab_size - 1;//防止越界
  }
}
...
void ReadWord(char *word, FILE *fin) {
  ...
  while (!feof(fin)) {
    ...
    if ((ch == ' ') || (ch == '\t') || (ch == '\n')) {
      ...
      if (ch == '\n') {
        strcpy(word, (char *)"</s>"); //</s> 代表EOS, 在词表中的索引为0
        return;
...
int GetWordHash(char *word) {
  ...
  for (a = 0; a < strlen(word); a++) hash = hash * 257 + word[a];//hash function
...
int SearchVocab(char *word) {
  ...
  while (1) {
    ...
    // return the index. 注意strcmp，字符串相等时返回0
...
int AddWordToVocab(char *word) {
  ...
  while (vocab_hash[hash] != -1) 
    hash = (hash + 1) % vocab_hash_size;//处理碰撞
  
  // Map the hash code to the index of the word in the 'vocab' array.  
  vocab_hash[hash] = vocab_size - 1; //因为vocab_size是逐词增加的
...
// Reduces the vocabulary by removing infrequent tokens滤除低频词
void ReduceVocab() {
  int a, b = 0;
  unsigned int hash;
 ...
 * in `point` should be trained to output 0 or 1.（left/right, or positive/negative)
 */
void CreateBinaryTree() {
  ...
  // vocab_size * 2 + 1 词表大小vocab_size，合并次数(非叶子节点个数）vocab_size-1
  // count的前半部分表示叶子节点的权重(词频)，后半部分表示非叶子节点的权重，还多出来两个是防止溢出？
  ...
  //     
  for (a = 0; a < vocab_size; a++) count[a] = vocab[a].cn; //前半部分设为词频
  for (a = vocab_size; a < vocab_size * 2; a++) count[a] = 1e15;//后半部分设为一个很大的正整数
   ...
   */  
  // 构造哈夫曼树：
  // 1.词表中所有的词都对应一个节点，以词频为权重，将每个节点看做1棵树，由此组成一个森林，每次
  // 2.从森林中选择两个根节点权值最小的树合并，作为一棵新树的左右子树，新树根节点的权值为其左右子树根节点的权值之和,
  // 3.然后删掉被选中的两棵树，并将新树加入森林
  // 4.重复2,3,　直至森林中只有一棵树，此树即为哈夫曼树
  // The number of tree combinations needed is equal to the size of the vocab_size-1.
  for (a = 0; a < vocab_size - 1; a++) {

    // First, find two smallest nodes 'min1, min2'　可以封装为一个函数，没必要写两次
    ...
    // Calculate the combined weight. We could be combining two trees.
    count[vocab_size + a] = count[min1i] + count[min2i];//count的后半部分保存合并后的权重
    
    // Store the path for working back up the tree. 记录父节点
    parent_node[min1i] = vocab_size + a;
    parent_node[min2i] = vocab_size + a;
    
    // binary[min1i] = 0; // This is implied.默认初始化就是0
   ...
   *    .point - A variable-length array of output row indeces.在输出矩阵的哪一行
  ...
  for (a = 0; a < vocab_size; a++) {
    b = a;//a表示叶子节点，b是用来寻路的，从leaf-->root
    ...
    while (1) {
      ...
      // 逆序的，point[0]表示叶子节点，其他point[i]表示输出矩阵的第i行
      point[i] = b;
      ...
      b = parent_node[b];//update node b
      
      // We've reached the root when...
      // count, binary, parent_node的大小都是2*vocab_size+1, 而所有的节点数为2*vocab_size-1
      if (b == vocab_size * 2 - 2) break; //root: 2*vocab_size-1-1
    }
    
    // Record the code length (the length of the `code` list).
    vocab[a].codelen = i;
    
    // The root node is at row `vocab_size - 2` of the output matrix. 
    // 把路径翻转过来：root-->leaf
    // 注意：vocab[word].point 与point的差别，前者的取值范围是0到vocab_size - 2, 后者的取值范围是0到vocab_size * 2 - 2
    // 所以，前者的root是vocab_size - 2，后者的root是vocab_size*2 - 2
    vocab[a].point[0] = vocab_size - 2;//point[0]原来是leaf，现在将其变为root
    
    // For each bit in this word's code... 把路径翻转过来：root-->leaf
    for (b = 0; b < i; b++) {
      ...
      // 将point[b]对应的索引转化为vocab[word].point[b-i]对应的索引, 减去前半部分叶子节点占用掉的vocab_size即可
...
void LearnVocabFromTrainFile() {
  ...
  // Populate the vocab table with -1s.用-1初始化
  for (a = 0; a < vocab_hash_size; a++) vocab_hash[a] = -1;
...
void InitNet() {
  ...
  // syn0'.　输入词向量(input vector): vocab_size * layer1_size, layer1_size代表词嵌入的维度/隐层神经元的个数
  ...
  if (hs) {
    // syn1: vocab_size * layer1_size, 表示采用hs时的输出向量(非叶子节点的向量表示，注意这里不能称作输出词向量，因为非叶子节点并不对应单词)
    a = posix_memalign((void **)&syn1, 128, (long long)vocab_size * layer1_size * sizeof(real));
    if (syn1 == NULL) {printf("Memory allocation failed\n"); exit(1);}
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
     syn1[a * layer1_size + b] = 0; //0初始化，注意2D矩阵是以1维数组的形式存储的
  }
  ...
  if (negative>0) {
    ...
    // syn1neg: vocab_size * layer1_size, 表示采用负采样时的输出词向量(正、负样本的词向量)
  ...
  // init from a random uniform distribution in the range [-0.5, 0.5) and divided by the embedding_size
  // see detail in https://www.quora.com/How-are-vectors-initialized-in-word2vec-algorithm
  for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++) {
    next_random = next_random * (unsigned long long)25214903917 + 11;
    syn0[a * layer1_size + b] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / layer1_size;
  }
  
  //这里有个问题，输入词向量syn0采用随机初始化，输出向量syn1, syn1neg都采用0初始化，why?
  //sigmoid(uv)表示u被预测为正的概率,u代表输出向量，v代表输入向量
  //通常我们都会对权重参数随机初始化,所以输入词向量随机初始化是很自然的（如果0初始化，就啥也学不到了，看谁都55开）
  //输出向量就不同了，输出向量初始化为全0,则sigmoid(uv)=0.5, 也就是说模型的初始状态几乎是瞎猜，got it?

  // Create a binary tree for Huffman coding.
  // 为何负采样算法也需要构建哈夫曼树？ 实际上负采样并未用到Huffman tree
  // 但hs与neg其实本质上是接近的，都是将原本的V分类问题转化为一系列的二分类问题
  // 比较一下二者的目标函数，从某种意义上来说我们可以通过哈夫曼树来实现neg(将正、负样本当做非叶子节点，叶子节点仍然对应词表中的词)
  // 从root走到leaf的概率就是在当前输入下输出为该leaf对应的词的概率
  CreateBinaryTree();
}
...
void *TrainModelThread(void *id) {
  ...
  // neu1 is only used by the CBOW architecture.　the average of the context word vectors
  real *neu1 = (real *)calloc(layer1_size, sizeof(real));
  
  // neu1e is used by both architectures. the (input) word vector increment
  ...
  while (1) {
    ...
    if (word_count - last_word_count > 10000) {
      ...
      // Don't let alpha go below [initial alpha] * 0.0001.学习率衰减、校正
      if (alpha < starting_alpha * 0.0001) alpha = starting_alpha * 0.0001;
    }
    
    // This 'if' block retrieves the next sentence from the training text and
    // stores it in 'sen'.
    // sentence_length初值为0,　单词是逐个加入的
    if (sentence_length == 0) {
      while (1) {
        ...
        /* 
         * =================================
         *   Subsampling of Frequent Words　高频词亚采样
         * =================================
         ...
         *   - ran = 0.033, the minimum (3.3% chance of being kept) when x = 1.
        ...
        if (sample > 0) {
          ...
          //　此处与原论文有出入，原论文采用的是丢弃概率1-sqrt(sample/x)
          // 这里是保留概率，但很明显1-ran != 1-sqrt(sample/x)
        ...
        // Verify the sentence isn't too long.截断过长的句子
    ...
    word = sen[sentence_position];//逐个窗口训练，word代表训练窗口的中心词
    
    if (word == -1) continue;
    
    for (c = 0; c < layer1_size; c++) neu1[c] = 0;//CBOW的输入向量：上下文词向量之平均，初始化为0，后面会赋值
    for (c = 0; c < layer1_size; c++) neu1e[c] = 0;//(输入)词向量的更新量，0初始化
    ...
    // 后面会在在1和max_window_size（最大背景窗口）之间随机均匀采样一个整数作为背景窗口大小
    b = next_random % window;//b为窗口缩减量，实际窗口为window-b,取值在1到window之间
    ...
    if (cbow) {  //train the cbow architecture
      ...
      // a != window means skipping the center word
      // 实际的窗口是b-->window * 2 + 1 - b, 即单侧从window变为window-b
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {

        // Convert the window offset 'a' into an index 'c' into the sentence 
        // array.
        c = sentence_position - window + a; //a-window是以0为中心的上下文词的位置
        ...
        // 内循环累计单个上下文词的词向量(词向量从输入词向量矩阵syn0获取)
        // 外循环结束后得到上下文词向量之和，后面会求平均
      ...
      if (cw) {
        ...
        //            rows to train on. 到达word的路径上的非叶子节点，对应输出矩阵的行索引
        //   .code - A variable-length list of 0s and 1s, which are the desired
        //           labels for the outputs in `point`.哈夫曼编码
        ...
        if (hs) for (d = 0; d < vocab[word].codelen; d++) {
          ...
          l2 = vocab[word].point[d] * layer1_size; //再次提醒，*layer1_size是因为矩阵是以1d形式存储的
          ...
          for (c = 0; c < layer1_size; c++) f += neu1[c] * syn1[c + l2];//计算输入向量和输出向量的点乘
          
          // Apply the sigmoid activation to the current output neuron.
          // expTable保存了预先计算好的激活值
          if (f <= -MAX_EXP) continue;//MAX_EXP=6
          else if (f >= MAX_EXP) continue;
          else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];//激活之后f表示预测为正样本的概率
          ...
          // back-propagate errors output -> hidden 累计词向量的更新量neu1e
          // neu1e后面会用来更新输入词向量syn0
          for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1[c + l2];
          // Learn weights hidden -> output 更新输出矩阵
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          ...
          // 为何在hs中f超出MAX_EXP时，可以忽略，这里却要算？         
          if (f > MAX_EXP) g = (label - 1) * alpha;//此时f激活之后非常接近1,这里直接用1替代
          else if (f < -MAX_EXP) g = (label - 0) * alpha;//此时f激活之后非常接近0，这里直接用0替代
        ...
        // CBOW：由上下文预测中心词，根据中心词的预测情况(error)来调整输入(上下文)词向量
        // 相当于一个老师(中心词)多个学生(上下文词)，根据一个老师的反馈来对多个学生进行调整
        for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
          // Convert the window offset 'a' into an index 'c' into the sentence 
          // array.
    ...
    // 每个位置的词最终会经过K2个老师的一对一教学，由于每个窗口的大小是随机采样的，K2会比K1更均匀，并且对某个位置的词的训练是在K2个窗口中逐步完成的，整体效果更好
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        // Get the context word. That is, get the id of the word (its index in
        // the vocab table).
        last_word = sen[c];//对skip-gram而言，上下文词是输出
        ...
        //CBOW中输入是多个词(上下文)，skip-gram的输入是单个词(本该是中心词，但这里悄然的做了一个输入输出的角色转换，以每个上下文词last_word为输入)
        l1 = last_word * layer1_size;
        // 对于窗口内的每个要预测的词，都需要重新初始化neu1e为0
        // 相当于1对1教学，每个学生配对一个老师
        ...
        if (hs) for (d = 0; d < vocab[word].codelen; d++) {
          f = 0;
          l2 = vocab[word].point[d] * layer1_size;//以中心词word为输出(原本应该是上下文词last_word)
          // Propagate hidden -> output
          for (c = 0; c < layer1_size; c++) f += syn0[c + l1] * syn1[c + l2];
          if (f <= -MAX_EXP) continue;
          else if (f >= MAX_EXP) continue;
          else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
          // 'g' is the error(1-code-f) multiplied by the learning rate
          g = (1 - vocab[word].code[d] - f) * alpha;
          // back-propagate errors output -> hidden
          for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1[c + l2];
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          // On the first iteration, we're going to train the positive sample.
          if (d == 0) {
            target = word; //注意这里是以中心词word(如果按原论文的意思应该是以上下文词last_word)为正样本
            label = 1;
          // On the other iterations, we'll train the negative samples.
          } else {
            // Pick a random word to use as a 'negative sample'; do this using 
...
void TrainModel() {
  ...
  // Either load a pre-existing vocabulary, or get the vocabulary from 
  // the training file.
  ...
  InitNet(); //分配词向量空间并初始化，构造哈夫曼树，当然了，负采样用不到哈夫曼树
  ...
  if (classes == 0) {
    ...
    // 如果要分类别，Run K-means on the word vectors
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < EXP_TABLE_SIZE; i++) {
    ...
    // is equivalent to 
    //    1 / (1 + exp(-x))
    expTable[i] = expTable[i] / (expTable[i] + 1);                   // Precompute sigmoid(x)
  }
